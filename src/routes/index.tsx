...
import { CalendarDays, CheckCircle2, CircleDollarSign, Clock, TrendingDown, Wallet, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Overview for {now.toLocaleDateString("en-US", { month: "long", year: "numeric" })}</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        <StatCard label="Total Bookings" value={active.length} icon={CalendarDays} tone="primary" />
        <StatCard label="Confirmed" value={confirmed} icon={CheckCircle2} tone="success" />
        <StatCard label="Available Slots" value={available} icon={Clock} tone="info" />
        <StatCard label="Total Paid" value={fmtMoney(totalPaid)} icon={Wallet} tone="success" />
        <StatCard label="Unpaid" value={fmtMoney(totalUnpaid)} icon={TrendingDown} tone="destructive" />
        <StatCard label="Deposits" value={fmtMoney(totalDeposit)} icon={CircleDollarSign} tone="warning" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 rounded-xl border bg-card">
          ...
          <div className="divide-y">
            {latest.length === 0 && <div className="p-6 text-sm text-muted-foreground">No bookings yet.</div>}
            {latest.map((b) => (
              <Link key={b.id} to="/bookings/$id" params={{ id: b.id }} className="flex items-center gap-3 p-4 hover:bg-accent/30 transition-colors">
                ...
                <div className="hidden sm:block text-right">
                  ...
                  <div className="text-xs text-muted-foreground">Remaining {fmtMoney(b.remaining_amount)}</div>
                </div>
        ...
        <section className="rounded-xl border bg-card">
          ...
          <div className="divide-y">
            {upcoming.length === 0 && <div className="p-6 text-sm text-muted-foreground">Nothing on the schedule.</div>}
            {upcoming.map((b) => (
              <Link key={b.id} to="/bookings/$id" params={{ id: b.id }} className="block p-4 hover:bg-accent/30">
                ...
                <div className="text-xs text-muted-foreground">
                  {b.slot && `${b.slot.date === today ? "Today" : "Tomorrow"} · ${slotTimeRange(b.slot.start_time, b.slot.end_time)}`}
                </div>
                {Number(b.remaining_amount) > 0 && (
                  <div className="text-xs text-destructive mt-1">Owes {fmtMoney(b.remaining_amount)}</div>
                )}
              </Link>
            ))}
          </div>
